...
### Step 2: Evaluate Conditional (If Present) Then Execute or Skip (MANDATORY)
1. If task has `[CONDITIONAL]` flag:
   - Read `**Quick Reference:**` section for authoritative condition rules (do not invent logic).
   - Evaluate required variable (e.g., `{readme_content}` for `@task-scan-readme`, `{commands_extracted}` for `@task-execute-readme`). Condition TRUE ⇨ non-empty trimmed value and not equal to "NONE".
   - If FALSE:
...
2. Execute the task if not skipped:
   ...
   - On FAIL: mark line `- [x]` and append task_status=FAIL; set execution_status=FAIL then go to Step 4.
   - On BLOCKED: mark line `- [x]` and append task_status=BLOCKED; set execution_status=BLOCKED then go to Step 4.
3. If flag `[NON-SCRIPTABLE]` present, ensure reasoning/tool-call execution (not autogenerated script) but output handling identical.
4. If flag `[MANDATORY]` present, no special ordering—processed strictly in listed sequence.

### Step 3: Iterate to Next Task (MANDATORY)
1. Re-read checklist file from disk (ensures persistence).
