@task-execute-readme repo_directory={{repo_directory}} repo_name={{repo_name}} commands_json_path={{commands_json_path}}@task-execute-readme repo_directory={{repo_directory}} repo_name={{repo_name}} commands_json_path={{commands_json_path}}@task-execute-readme repo_directory={{repo_directory}} repo_name={{repo_name}} commands_json_path={{commands_json_path}}
...
This task MUST be performed using DIRECT TOOL CALLS and STRUCTURAL REASONING:** CRITICAL ** DO NOT GENERATE OR EXECUTE A SCRIPT FOR THIS TASK.This task MUST be performed using DIRECT TOOL CALLS and STRUCTURAL REASONING
...
0. DEBUG Entry Trace: Use run_in_terminal with echo/Write-Host:2. Use structural reasoning to classify each command as SAFE or UNSAFEBehavior:

   "[debug][task-execute-readme] START repo_directory='{{repo_directory}}' commands_json_path='{{commands_json_path}}'"

3. Use run_in_terminal tool to execute SAFE commands individually1. Use read_file tool to load commands from task-scan-readme output JSON

1. Input Parameters:

   - repo_directory: absolute path to repository root4. Use create_file tool to save execution results to JSON output

   - repo_name: repository name

   - commands_json_path: path to the JSON file containing extracted commands (from task-scan-readme output)5. Use replace_string_in_file tool to update progress/results filesThis task MUST be performed using DIRECT TOOL CALLS and STRUCTURAL REASONING:2. Use structural reasoning to classify each command as SAFE or UNSAFE
...
2. Prerequisites Check:
   ...
   - Use read_file to load the file specified in commands_json_path parameter** NEVER create a Python/PowerShell/Bash script for this task **1. Use read_file tool to load commands from task-scan-readme output JSON3. Use run_in_terminal tool to execute SAFE commands individually

   - Extract the commands_extracted array from the JSON

   - If task-scan-readme was SKIPPED (no README) or FAIL, skip execution** NEVER batch-execute commands in a single script **

   - If DEBUG=1, print: `[debug][task-execute-readme] loaded {{command_count}} commands from {{commands_json_path}}`

   - If no commands or scan failed, set status=SKIPPED and proceed to output** EACH command must be evaluated individually for safety **2. Use structural reasoning to classify each command as SAFE or UNSAFE4. Use create_file tool to save execution results to JSON output
...
3. **Structural Reasoning - Safety Classification**:
   ...
   The AI agent must use reasoning to determine command safety and execute safe commands one at a time.3. Use run_in_terminal tool to execute SAFE commands individually5. Use replace_string_in_file tool to update progress/results files
